...
import { LocalImage, String, Button, Subjects, Dash } from '../../reusables'
...
class BookedReview extends Component {
	constructor(props) {
		super(props)
		this.state = {
			...
			<ScrollView
				...
				<View style={styles.container}>
					...
					<View
						...
						{!!this.props.selectedAppointment &&
							!!this.props.selectedAppointment.reviewschedule &&
							!!this.props.selectedAppointment.reviewschedule
								.length &&
							this.props.selectedAppointment.reviewschedule.map(
								(schedule, key) => {
									let schedObject = programSchedule(schedule)
									let date = schedObject.date
										.toString()
										.split(' ')
									let morning, afternoon
									if (
										!!parseInt(schedObject.morningDuration)
									) {
										let hours =
											parseInt(schedObject.morningTime) /
											1
									...
									if (
										!!parseInt(
											schedObject.afternoonDuration
										)
									) {
										let hours =
											parseInt(
												schedObject.afternoonTime
											) / 1
										let minutes = Math.floor(
